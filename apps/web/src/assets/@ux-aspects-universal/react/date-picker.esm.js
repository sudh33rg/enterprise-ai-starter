...
function CalendarGrid() {
  ...
  const calendarGridLayout = useMemo(() => {
    return weeks.map(weekdays => React.createElement(CalendarGridRow, {
      ...
      date,
      key
    }) => React.createElement(DateButton, {
      date: date,
      key: key
    }))));
...
function DatePickerHeader({
  min,
  max,
  nextMonthBtnAriaLabel,
  previousMonthBtnAriaLabel
}) {
  const {
    isJato
  } = useThemeFamily();
  ...
  const {
    nextMonthBtnAriaLabel: defaultNextMonthBtnAriaLabel,
    previousMonthBtnAriaLabel: defaultPreviousMonthBtnAriaLabel,
    monthYearOrder
  } = useDatePickerI18nContext();
...
function DatePicker(_a) {
  var {
    ...
    props = __rest(_a, ["value", "min", "max", "isDisabled", "isDateDisabled", "previousMonthBtnAriaLabel", "nextMonthBtnAriaLabel", "onValueChange", "autoFocus"]);
...
function DateRangeCalendar() {
  ...
  const {
    ...
    endPickerAriaLabel
  } = useDateRangePickerI18nContext();
  const {
    focusWithinProps: startFocusWithinProps
  } = useFocusWithin({
    onFocusWithin: () => announce(startPickerAriaLabel)
  });
  const {
    focusWithinProps: endFocusWithinProps
  } = useFocusWithin({
...
function DateRangePicker(_a) {
  var {
    ...
    props = __rest(_a, ["start", "end", "min", "max", "isDisabled", "isDateDisabled", "showSummary", "onValueChange"]);
  const {
    isJato
  } = useThemeFamily();
