...
const TreeDrawer = ({ isMinimized, onMinimizedChange, isOpen = true, size = 256, unit = "pixel", mode = "adjacent", placement = "start", resizable = false, resizeHandleAriaLabel, minSize, maxSize, onSizeChange, children }) => {
...
const TreeHeader = ({ minimizeButton, children }) => {
    const { isMinimized } = useTreeDrawer();
...
const TreeItemLink = ({ icon, children }) => {
    return React.createElement("div", {
...
const TreeMinimizeButton = _a => {
    ...
    const { toggleMinimized, isMinimized } = useTreeDrawer();
...
const TreeNodeExpander = _a => {
    var props = __rest(_a, []);
    const { toggleParent, level } = useTreeItem();
...
function TreeNode(_a) {
    var _b, _c;
    var { icon, expander, content, model, isSelectable = true, loadingIndicator, trailingAction, className } = _a, props = __rest(_a, ["icon", "expander", "content", "model", "isSelectable", "loadingIndicator", "trailingAction", "className"]);
    const { isMinimized, toggleMinimized } = useTreeDrawer();
    const { focusProps, isFocusVisible } = useFocusRing();
    const ref = useRef(null);
    const { level, collapseParent } = useTreeItem();
    const { tabIndex, tabbableProps } = useTabbable(ref);
    const { focusNext } = useFocusManager();
    const { treeControl, SecondaryItem, selectionManager, selections, isRtl, loadChildren } = useTreeContext();
...
function Tree({ treeControl, node, selectionManager, loadChildren }) {
