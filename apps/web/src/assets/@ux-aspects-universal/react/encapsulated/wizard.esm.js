...
function useWizard({ steps, activeStep, onFinish, onActiveStepChange }) {
...
function WizardStepButton({ step }) {
    const ref = useRef(null);
    const { id, label, status, statusIndicator } = step.props;
    const { activeStep, isStepVisited, goToStep } = useWizardContext();
    const { focusProps, isFocusVisible } = useFocusRing();
    const { tabIndex, tabbableProps } = useTabbable(ref);
    const { successStatusLabel, warningStatusLabel, errorStatusLabel, activeStatusLabel, pendingStatusLabel } = useWizardI18nContext();
...
const WizardNextButton = ({ children, visible }) => {
    const { isLastStep, goToNextStep } = useWizardContext();
...
const WizardPreviousButton = _a => {
    ...
    const { isFirstStep, goToPreviousStep } = useWizardContext();
...
const WizardFinishButton = _a => {
    var { children, visible } = _a, props = __rest(_a, ["children", "visible"]);
    const { isLastStep, onFinish } = useWizardContext();
