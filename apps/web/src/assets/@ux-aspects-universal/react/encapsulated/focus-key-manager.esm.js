...
function ActiveDescendantProviderFn({ children, onEnter, loop = false }, ref) {
...
function useActiveDescendantItem({ id, disabled = false, value }) {
    id = useUniqueId("uxa-active-descendant-item", id);
    const ref = useRef(null);
    const { registerItem, unregisterItem, activeId, setActiveItem } = useActiveDescendant();
    useEffect(() => {
        registerItem({
            id,
            disabled,
...
const LegacyActiveDescendantKeyManager = React.forwardRef(({ isOpen, children, listRef }, ref) => {
    ...
    const setActiveDescendant = useCallback(({ id, origin }) => {
...
function FocusKeyManager(_a) {
    var { inheritFocusManager = false } = _a, props = __rest(_a, ["inheritFocusManager"]);
...
function InternalFocusKeyManager({ wrap, autoFocus, orientation, columns, onGoToNextPage, onGoToPreviousPage, comparator = (a, b) => a === b, children, sort = defaultSort, controller }) {
    const items = useRef([]);
