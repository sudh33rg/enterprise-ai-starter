...
const BreadcrumbLink = forwardRef((_a, ref) => {
    var { index, isLast, separator, children, isOverflowing } = _a, props = __rest(_a, ["index", "isLast", "separator", "children", "isOverflowing"]);
    const internalRef = useRef(null);
...
const Breadcrumbs = _a => {
    var _b;
    var { separator, size = "medium", children, maxBreadcrumbs = Infinity, isResponsive, overflowButton = () => React.createElement(BreadcrumbOverflowButton, null) } = _a, props = __rest(_a, ["separator", "size", "children", "maxBreadcrumbs", "isResponsive", "overflowButton"]);
    if (maxBreadcrumbs < 2) {
    ...
    const { leadingBreadcrumb, menuBreadcrumbs, trailingBreadcrumbs } = useResponsiveBreadcrumbs(breadcrumbsRef, children, maxBreadcrumbs, isResponsive);
    ...
    const { ref: menuRef } = triggerProps, menuTriggerProps = __rest(triggerProps, ["ref"]);
...
const BreadcrumbOverflowButton = forwardRef((_a, ref) => {
    ...
    const { tabbableProps, tabIndex } = useTabbable(internalRef);
    const { isJato } = useThemeFamily();
    const { overflowMenuButtonAriaLabel } = useBreadcrumbsI18n();
