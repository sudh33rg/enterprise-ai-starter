...
const ToggleSwitch = forwardRef((props, ref) => {
    const { id, className, style, isDisabled, isSelected, onValueChange, children, value = "on", defaultSelected, labelPosition = "after" } = props, htmlProps = __rest(props, ["id", "className", "style", "isDisabled", "isSelected", "onValueChange", "children", "value", "defaultSelected", "labelPosition"]);
    const state = useToggleState(Object.assign(Object.assign({}, props), {
        onChange: onValueChange,
        defaultSelected
    }));
    const inputRef = useRef(null);
    const { inputProps } = useSwitch(Object.assign({
        isSelected,
        onChange: onValueChange,
        value
    }, props), state, inputRef);
    const { isFocusVisible, focusProps } = useFocusRing();
    const { wrapperId, inputId } = useWrappedInputId(id, "uxa-toggle-switch");
