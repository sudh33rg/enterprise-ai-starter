...
const Table = forwardRef((_a, ref) => {
  var {
    ...
    props = __rest(_a, ["className", "size", "isEmbedded", "isStriped", "children"]);
...
const TableRow = forwardRef((_a, ref) => {
  var {
    ...
    props = __rest(_a, ["isSelected", "children"]);
  const {
    region
  } = useContext(TableRegionContext);
  const {
    focusProps,
    isFocusVisible
  } = useFocusRing();
  const _b = mergeProps(focusProps, props),
    {
      className
    } = _b,
    mergedProps = __rest(_b, ["className"]);
  return React.createElement("div", Object.assign({
    ref: ref,
    role: "row",
...
const TableHeaderCell = forwardRef((_a, ref) => {
  var {
    ...
    props = __rest(_a, ["isSortable", "sortDirection", "onSortDirectionChange", "align", "children"]);
  const {
    isJato
  } = useThemeFamily();
